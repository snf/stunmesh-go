...
// A smaller, fresh-corpus continuation keeps YAML minimization bounded after
// the broad filtered campaign spent long periods minimizing 600+ byte cases.
// The limit is a deliberate audit scope, not a production size restriction.
func FuzzAuditConfigLoaderFilteredSmall(f *testing.F) {
	f.Add([]byte("refresh_interval: 60s\n"))
	f.Add([]byte("interfaces:\n  wg0:\n    protocol: ipv4\n    peers: {}\n"))
	f.Add([]byte("plugins:\n  store:\n    type: builtin\n    name: opendht\n"))
	f.Add([]byte("refresh_interval: [bad, shape]\n"))
