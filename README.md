...
[DIRECT_ACCESS_AND_SYNC_PLAN.md](DIRECT_ACCESS_AND_SYNC_PLAN.md) is the subsequent preparation-only plan for direct laptop access, Syncthing on the large encrypted volume, LAN/VPN service paths and disabled weekly encrypted cloud backups. It also documents a later whole-LAN mapping through `10.77.1.x`, preserving the initial service addresses. It distinguishes existing commands from helpers still to implement; credential rotation and extended reliability tests are deferred.
