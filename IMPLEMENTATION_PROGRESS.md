...
Complete all locally possible implementation, tests, release APK and container builds before handoff. No deployment, firewall change, NAS ownership change, production key rotation or real phone reset. NAS/GrapheneOS access arrives later: hardware Keystore/backup, real NAT, battery measurements and rootless host integration must be identified as pending device checks, never claimed passed locally. Publication is handled separately after credential and privacy review.
...
- Preserve the implemented security fixes, pinned dependencies and newer Linux client work. Runtime source corresponding to the signed APK and published OCI images is unchanged by history sanitization.
