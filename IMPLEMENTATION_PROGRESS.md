...
- Persisted-read root cause reproduced on synthetic public/device profiles: Android JSONObject emits standard JSON solidus escapes (`\/`), rejected by the pinned SnakeYAML scanner. Hardware decryption passed; no owner secret was exported or profile cleared. Android fix 03aa918 extends only the scanner escape table before parsing, preserving byte/depth/type/duplicate-key limits. Added literal-backslash/duplicate-key/invalid-escape regressions and a real encrypted disk round-trip instead of cached reload. Release code 4 and device regression build are in progress; no dependency changes.

- Release4/code4 (Android 3cfcb61, Go d4d12e9) passes fresh offline 135-task build, 12+12 JVM tests, lint, signature/native/alignment checks; real hardware/disk round-trip 2/2 passes. Compatible update automatically reloads the original encrypted phone profile and authenticates with the unchanged public key. Distinct-app split-route/32KiB/HTTPS probe passes1/1; HTTP canary passes. Updated hotspot initially timed out, then NAS reported new external WG handshake at16:01:33UTC on the existing refresh cycle. Requested browser confirmation and return-home test; do not infer completed handover acceptance yet.

- Updated end-to-end roaming passes: owner confirmed hotspot HTTP, then about10second home-Wi-Fi recovery without restarting STUNMESH. NAS WG timestamps16:01:33UTC external and16:04:20UTC home independently confirm authenticated paths; outbound switch time unobserved, discovery delay remains. NAS closeout0f1b00d restores info logging and removes temporary TCP canaries/files (absence verified); monitor stopped. Waiting on the changed Wi-Fi ADB port to restore original official WireGuard always-on/lockdown-off through normal settings.

- Interactive session closeout complete: official WireGuard connected on tun0/UID10161, always-on restored and lockdown off; STUNMESH service stopped with original enrolled profile retained. Wireless debugging disabled through normal UI; TCP42637 closed, local ADB server stopped and only temporary diagnostic credentials retired. Owner release signer retained. NAS trial remains running with info logs and no diagnostic TCP listeners; original Samba/upstream VPN/NFS and host firewall unchanged. Remaining acceptance gates are service integration, encrypted OS restore, IPv6, reboot/Doze and measured battery use.

## Signed release packaging and credential review — 2026-09-22

- Owner requested commits in both repositories and the tested signed APK. Added the unchanged `0.3.0-local.4` release to the Android repository under `releases/`, with checksum, signer/build provenance and links to validation. Actual build-source commits remain unchanged in the manifest. No new build, app/server code change, push or GitHub release publication.
- Reviewed all locally available Git objects/history, current files, historical archive contents and all 139 APK entries; compared against the actual local enrollment PSK and signing credentials without exposing values. No real credential detected; remaining matches are synthetic test inputs, placeholders, public peer keys and dependency hashes. `SECRET_REVIEW.md` records scope, counts and limitations. No compromised credential was identified for rotation.
- Added verified ignore guards for local signing/access/enrollment material and a single reviewed APK exception. Final staging uses explicit paths and receives a separate credential/content check; private signing files and confidential enrollment JSON/QR remain outside both repositories. Existing device/build results continue to apply to the identical APK; outstanding service, OS recovery, IPv6, Doze/reboot and battery gates remain open.

## Headless laptop client and NAS service access — 2026-09-22

- Added rootless client deployment/configuration examples reusing the reviewed image without runtime/source/dependency changes. External Podman client passed WG authentication, real SSH login/concurrency, SMB negotiation, split routing/ordinary internet and client-restart recovery. Two minimal VPN-bound NAS forwarders expose existing SSH/SMB with no host firewall/route or service-data changes.
- Retired the temporary VPS client and peer, enrolled a fresh laptop-only identity, and prepared its private external configuration. Owner requested plain Podman commands to run on the actual laptop host because the current restricted workspace cannot start the nested networked client. `CLIENT_PROGRESS.md` records evidence, cleanup, remaining privileged routing/Syncthing stages and the Samba credential exposure requiring rotation. No real credentials enter either source repository.
