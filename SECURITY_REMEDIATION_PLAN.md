...
**Updated:** 2026-09-22 · **Revision:** destination-only split tunnel, non-backupable phone identity, public-information QR provisioning. **Status:** plan changes only; no remediation or deployment performed.
...
- Android configuration and identity are **intentionally non-backupable**. No Android/GrapheneOS backup or restore adapter. After restore, reinstall/data loss or explicit re-enrollment, add the phone as a new peer with a newly generated local private key; retire the previous server peer.
- The phone private key must not be displayed, exported or passed to any other application, in plaintext or wrapped form. Generate it on the phone; use the strongest verified hardware-backed storage available. F8/N2 explicitly distinguish hardware protection at rest from the current WireGuard engine's necessary in-process key use; do not claim complete hardware confinement.
- F18's external-scanner/local-encoder QR approach is selected, adapted to the no-private-key-transfer rule: QR carries public bootstrap information, and the phone returns only its public key. No private-key provisioning QR or recovery export.
...
Design decisions **D1–D4 are settled**. F8/N2 now prohibit configuration backup, superseding the earlier restore requirement; F18 is selected with public-only QR payloads. **F1–F18 retain their IDs**. Superseded alternatives are removed. Section 7 contains remaining hardware feasibility/provisioning inputs, not previously settled choices.
...
Use one stable fork application ID and APK signing identity, increasing versionCode on updates. Choose them before first installation; changing identity can require reinstall/re-enrollment. A distinct ID avoids mixing upstream and local installations. Protect/back up the **workstation APK signing key** outside the build workspace; this is separate from the phone WG private key, which must never be backed up. Local signing still trusts the tools and workstation.
...
Retain useful public configuration syntax, but reject private-key-bearing Android imports under F16; this deliberately narrows legacy profile compatibility. Internal Go/WG configuration still needs typed validation. Upgrading YAML/mapstructure alone is insufficient. Parse only on import/configuration change.
...
### F8. Preserve local configuration; forbid backup and require a new peer after restore

**SETTLED · Android only · Medium data-loss/secret-exposure risk · A-05/A-18, N2 · Complexity M storage, S backup exclusion.**

**Risk:** read/decrypt errors currently become an empty store that can overwrite good data; failed replacement can delete the previous file, and stale UI state can overwrite active selection. Backup can copy sensitive configuration or restore unusable ciphertext. The earlier native-backup requirement is withdrawn.

**Local persistence:** keep one repository/state flow, serialized updates and `AtomicFile` rollback. Distinguish absent data from corruption/Keystore failure; preserve unreadable data privately and stop with an explicit error, never silently overwrite it. Move live configuration, private-key ciphertext and atomic temporary/rollback files into app-private `noBackupFilesDir`. Keep settings/diagnostics separate from secret access, so opening the UI need not unwrap the private key.

**Key ownership/storage:** create a fresh WG private key on the phone at enrollment with existing reviewed cryptographic primitives; never import a private key generated by the server/configuration tool. Store it encrypted with AES-GCM under a non-exportable Android Keystore wrapping key. Request StrongBox when supported for that algorithm, otherwise verified TEE-backed storage; check `KeyInfo.getSecurityLevel()` (or the appropriate older API) rather than claiming hardware backing from the Keystore name alone. Record only the security level, never key/blob contents. Do not silently fall back to a file-based wrapping key or software-only storage; report an unsupported device for review. [Android hardware-backed storage](https://developer.android.com/privacy-and-security/keystore), [KeyInfo verification](https://developer.android.com/reference/android/security/keystore/KeyInfo#getSecurityLevel()).

**Essential feasibility limit:** the current `wireguard-go` engine accepts a raw private key through UAPI and performs X25519 using that key in process memory. Its API does not accept an Android Keystore key handle. STUNMESH `mobile/uapi.go` likewise renders `private_key=`. Hardware wrapping therefore protects the key **at rest**, but the app must unwrap it for its embedded WG engine while running. This is within the same app, not a transfer to another application; nevertheless it is **not** a guarantee that the WG key never leaves secure hardware or survives a compromised app/OS. [Pinned WireGuard key operations](https://git.zx2c4.com/wireguard-go/tree/device/noise-helpers.go?id=f333402bd9cb).

If the requirement is literal hardware-confined WG operations with no plaintext key in app memory, the selected engine does not meet it: treat that as an unresolved deployment constraint, not a feature delivered by wrapping. Do not introduce a custom handshake, claim unsupported hardware X25519 compatibility, or undertake a new engine integration without a separate decision. The implementer must state which guarantee is achieved.

**No ordinary extraction path:** remove private-key display/edit/copy, full-profile exports, QR serialization and sharing endpoints. Never send the raw key or its encrypted blob to clipboard, keyboard/IME, document provider, external scanner, intents, logs, crash reports, Git or a backup transport. No exported provider/service may read it. Keep release builds non-debuggable; minimize secret-bearing string copies/retention during the unavoidable in-app WG handoff, and clear mutable buffers where practical without promising complete memory erasure. F16/F18 are narrowed accordingly.

**Backup exclusion:** set `android:allowBackup="false"`; exclude configuration/secret files and legacy paths from both old backup rules and Android 12+ cloud/device-transfer rules. Android notes that some devices allow device transfer despite `allowBackup=false`, so the private `noBackupFilesDir` placement and explicit exclusions are necessary. No `BackupAgent`, backup scheduler, `dataChanged()` calls, restore converter or key export. The APK may be reinstalled by the OS, but the peer identity/configuration must not return. [Android backup/exclusion behavior](https://developer.android.com/identity/data/autobackup).

**Restore/re-enrollment:** start unpaired after restore/reinstall/data clearing or loss of the wrapping key. Explain the state and require deliberate new enrollment; generate a different keypair and have the owner remove the old public-key peer from the server. A normal in-place signed app update preserves the existing local identity. Corruption must not trigger silent key rotation. Reject historical restored profiles/backup blobs as an enrollment source; do not resurrect their private keys. Existing external copies cannot be revoked retroactively: retiring the old server peer removes their authorization. Preserve server Git history as instructed.

**Files/acceptance:** repository/model, enrollment UI, `mobile` key handoff, manifest and backup-rule XML. Test atomic failure/stale-active-selection regressions and hardware security-level verification. Inspect OS/GrapheneOS backup and device-transfer output with synthetic material: neither key, wrapped blob nor configuration is present. Restore a test installation and verify it is unpaired; re-enroll, observe a different public key and require server authorization, with the retired peer rejected. Test loss of Keystore access, reboot/first unlock, in-place updates and all export surfaces. No real phone reset or server revocation is authorized by this document; use synthetic peers/test installation first.
...
**Recommended:** make advanced/multi-store profiles read-only in the current single-store editor with a clear reason; preserve all fields during supported edits. Any retained import/public export must follow F16's no-private-key-transfer contract. This is the smallest safe behavior for an OpenDHT-only home setup.

Retain this small safe restriction; no full multi-store editor. Preserve selected destination routes and public enrollment metadata during supported edits. Private-key ownership is controlled by F8, not the editor.
...
### F16. Remove secret export and private-key input; keep only public sharing

**SETTLED, supersedes the former plaintext-export recommendation · Android only · A-13/A-23 plus F8/N2 · Complexity S.**

**Risk:** the current full-profile export sends a WG private key/PSK to a document provider; the private-key form can expose it to an IME despite visual masking. A warning does not meet the new no-transfer requirement.

**Fix:** delete the full-secret profile export, private-key editor/reveal/copy controls and encrypted-blob sharing. Generate the phone identity locally under F8. Reject private-key-bearing external imports and provisioning payloads; do not silently accept a server-generated phone identity. Core validation from F1 remains mandatory for the internal configuration path.

Retain only an allowlisted public peer description (public key, required tunnel addresses and non-secret enrollment metadata) for server enrollment. It must never serialize a full `TunnelConfig` and then try to redact secrets. Diagnostics remain separate under F11. Use password keyboard options/autocorrect off for any remaining shared-secret input; no private-key keyboard field remains. PSKs are not public provisioning data and must not enter the public QR flow.

**Files/acceptance:** `MainActivity.kt`, `TunnelListScreen.kt`, `TunnelEditorScreen.kt`, public export/import serializer and model boundaries. Enumerate every export/share/copy/provider/debug path and verify no private key, wrapped-key blob or PSK leaves it. Test that private-key-bearing legacy YAML/wg-quick profiles are rejected, and public enrollment output contains only the declared fields. F18 reuses that public-only contract.
...
### F18. Public-information QR provisioning with local phone key generation

**SETTLED: external-scanner/local-encoder approach, adapted to F8/N2 · Cross-codebase · Complexity S–M.**

**Decision:** keep the recommended small QR workflow: the local configuration tool generates a QR, GrapheneOS Camera scans it, and the user explicitly imports its text. The payload is now **public bootstrap information only**. The former variant containing a workstation-generated phone private key is prohibited. No in-app camera SDK, extra Android permission, enrollment server or new authentication protocol. [GrapheneOS Camera QR support](https://grapheneos.org/usage#camera).

**Minimal enrollment flow:**

1. The owner's local tool emits a compact versioned bootstrap record: server WG public key, intended server/tunnel addresses and narrow phone-side destination routes, plus required public STUN/OpenDHT settings. No server/phone private key, encrypted private-key blob or PSK. Mark the proposal with an identifier so replies can be matched; it is not an authenticator.
...
QR is trusted local configuration input, not peer authentication. Treat a substituted bootstrap/server public key as an enrollment attack; user review/out-of-band comparison is necessary. No DHT value or automatic enrollment endpoint can authorize a new peer. **PSK handling:** public QR never carries it. If the chosen profile requires a WG PSK, retain that requirement and supply it through a separately agreed protected local provisioning path before activation; do not silently remove it to make QR enrollment work. Whether new profiles require a PSK remains a provisioning input.

**Implementation:** one-shot configuration-tool encoder plus an explicit public-text importer reusing the existing parser/validation. Verify whether a reviewed pinned encoder is already available; if not, select one small local dependency and pin its version/hash. Keep it out of the long-running Go/Podman image and Android runtime. No online QR generation, multipart QR format, autonomous device registration or general-purpose exported import receiver. Require explicit review/save, cap payload size and leave failures unpaired.

**Acceptance:** offline scan/import of a synthetic public record; private/PSK fields, unknown commands, oversized or broad-route payloads rejected; duplicate proposals cannot replace an existing private key silently. Inspect scanner/clipboard/document outputs and logs: they contain public enrollment data only. Verify phone private-key generation is local, successful activation requires the server's explicit peer change, and re-enrollment never reuses an exported/restored identity. F17 remains dependency triage; this feature's changes get their own paired Git commits when implemented.

## 3A. Android battery and background-work contract — applies to every fix

| Area | Required behavior |
| --- | --- |
| Tunnel off | No STUN/DHT requests, status polling, wake locks or reconnect jobs. Unregister callbacks and cancel Go/Kotlin work. No configuration-backup job or callback exists. |
...
| Validation / F1–F2, F6–F9 | Parse/validate and compare authorization state on configuration mutations. No timed config scans, app enumeration or continuous full-device rebuilds. No backup signals; avoid secret unwraps on status refresh. |
| UI / F10–F12, F16/F18 | Event-driven status/notification; visible-screen-only counter refresh; bounded log ring. Public-only provisioning/sharing occurs on user action. No telemetry or background scanner; key generation/Keystore work only when needed. |
...
## 4. Maintenance — deferred except mandatory key protection and backup exclusion
...
| **N2 — A-05: intentionally non-backupable phone private key** | Export, backup or device transfer could copy the identity. Keystore wrapping protects storage but the current WG engine still uses the key in app memory. | **Required under F8/F16.** Generate locally; strongest verified hardware-backed wrapping; no private-key or wrapped-blob export to another app, cloud backup, GrapheneOS backup or device transfer. Delete backup support. Restore/re-enrollment creates a new peer, never recovers this key. | M; verify hardware level, no-backup placement/exclusions and absence of export paths. Literal hardware-confined WG operations remain an explicit feasibility limit, not a claimed feature. |
...
| --- | --- | --- |
| **0 — Baseline and feasibility** | Preserve audit commits/evidence; create local branches. Carry D1–D4 and no-backup/public-QR decisions forward. Record destinations, app/signing identity and device storage capabilities; resolve section 7. | Distinguish hardware protection at rest from hardware-confined WG operations before claiming the security requirement is met. No app-filter or backup-adapter work. |
...
| **3 — Android storage/lifecycle** | F8–F12/F16 and required N2, with F5/F10 lifecycle. Delete private export/input; implement local identity ownership and backup exclusion. | Durable configuration, verified storage level, no ordinary key extraction/transfer path, foreground service and safe diagnostics. Restored installation is unpaired. |
...
| **5 — Provisioning/integration** | Implement F18 public-only QR flow after F1/F2/F8/F16; test synthetic peers, rootless namespace, VPS and actual Android carrier/home NAT. | Destination-only routing, local key generation, no secret in backup/scanner output, explicit new-peer enrollment, handover/idle and battery tests. |
| **6 — Review and staged use** | Review diff, deferred risks, hardware limitation and hashes; agree exact NAS/peer commands and manually install the local APK. | Existing LAN access preserved; public-only provisioning and explicit peer retirement tested. No public publication step. |
...
**Phone handoff:** local signed APK, hash/certificate, manual update instructions, public bootstrap QR and actual server routes. Document local key generation, public-key return, server authorization and re-enrollment/retirement steps. Test direct internet versus WG traffic, both families, handover, idle, reboot, metered sync and backup exclusion. Record OS version and actual Keystore security level. Demonstrate that restoring a test installation cannot recover its identity; do not reset the real phone without a separately agreed procedure.

**Rollback:** preserve server configuration history and exact build artifacts; never create a phone-private-key export as a rollback aid. If no known-safe Android version exists, stop the experimental tunnel and retain LAN administration. Normal app updates preserve the local identity; restore/re-enrollment deliberately replaces it and requires retiring its old server peer. Other unrelated production keys are not rotated by this plan.
...
| --- | --- |
| **Hardware guarantee (F8/N2)** | Verify StrongBox/TEE support on the phone. The existing engine requires an in-process raw WG key; hardware-backed wrapping is not hardware-confined WireGuard. If no plaintext outside secure hardware is mandatory, this remains an unmet deployment requirement requiring a separate engine/design decision. Do not silently substitute software-only protection. |
...
| **QR details / PSK (F18)** | Use public-only bootstrap + phone-generated key + public-key return. Choose a small pinned workstation encoder. Confirm whether new peer profiles require a PSK and, if so, its protected local provisioning path; public QR must not contain it or silently disable an existing PSK requirement. |
| **Installation identity and test inputs** | Record stable fork application ID, workstation signing-key location/fingerprint, phone OS/security level and synthetic backup-exclusion/re-enrollment procedure. The workstation signing key is distinct from the non-backupable phone WG identity. |
...
**Settled removals:** application-selection features, configuration backup/restore support, private-key/secret profile export, and private-key-bearing QR/import. No native backup adapter or backup-encryption design remains. General N1/N3 and nonessential N2 hygiene stay deferred; N2 key protection/exclusion is mandatory.
...
**Residual risks:** compromised endpoints/OS/WireGuard/toolchain or local signer; necessary private-key presence in the current WG process; public metadata and discovery DoS/replay; NAT failure and sleep-delayed recovery; deferred DNS behavior; substituted public enrollment records; and intentional loss of phone identity after restore. QR, backup exclusion and hardware wrapping do not remove the stated engine-memory limitation.
