...
**Implemented and locally validated; now deployed to an isolated NAS trial and tested on the provided Android 13 phone. Updated LAN authentication, split routing, persisted storage and data transfer pass. The prior release passed an external hotspot path and bounded restart; updated roaming, encrypted OS recovery, service integration and battery gates remain open.** Both repositories use `security-remediation/2026-09-22`; original audit branches/history remain. `ARTIFACT_MANIFEST.json` records the exact commits used to build each artifact; later test/documentation commits do not change those already built binaries. Artifacts are local under `../stunmesh-build/artifacts/`, not pushed or published.
...
Clean offline release4 build: 135 tasks, 12 debug +12 release tests, release lint, unchanged owner signer, verified native contents/16KiB alignment. Physical hardware/storage tests pass2/2 (0.983s); signed release restores the original encrypted identity after update; live split-route app probe passes1/1 (0.552s), and the tunnel HTTP canary passes. Updated hotspot→home retest remains pending. Exact provenance/current limits are in `ARTIFACT_MANIFEST.json` and `DEVICE_TEST_RESULTS.md`; physical evidence is under `device-test-evidence/2026-09-22/handover*`. No new background worker, permission, host firewall or service-share change.
