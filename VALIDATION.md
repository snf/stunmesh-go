...
| Real release APK | Owner-signed `dev.stunmesh.local`, version `0.3.0-local.1` / code `1`; non-debuggable, not test-only, no stub. Signature v3 verified with Android apksigner. ARM64/x86-64 `libgojni.so` exactly matches the local AAR; all native LOAD segments and APK ZIP layout support 16 KiB pages. |
...
| F18 | Shared public-only proposal/reply schema and fixture, 2048-byte QR bound, external scanner/local encoder, phone key generation, optional protected PSK input, explicit reviewed server peer enrollment. No private key/PSK QR or automatic authorization. |
