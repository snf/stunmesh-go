...
| Android offline source build | Clean committed snapshot, fresh output/project-cache directories, no build cache/network: **135 tasks executed**, release/debug APKs plus instrumentation APK built. Debug **12/12**, release **12/12** JVM tests pass; release lint passes. |
...
| Real release APK | Owner-signed `dev.stunmesh.local`, version `0.3.0-local.4` / code `4`; non-debuggable, not test-only, no stub. Signature v3 verified with Android apksigner. ARM64/x86-64 `libgojni.so` exactly matches the local AAR; all native LOAD segments and APK ZIP layout support 16 KiB pages. |
...
- Pixel 4a / Android 13: both instrumentation tests pass (0.837 s), including hardware wrapping/tamper rejection and a synthetic PSK-bearing enrollment, locally generated identity, encrypted write/cached reload and public-only reply. This older test did not validate a fresh disk read; release4 adds that regression below. Only the test-created profile is removed; no production app state cleared. Release `run-as` is denied as non-debuggable.
- Actual NAS rootless Podman starts the pinned image with NET_ADMIN only, correct UID/GID map, read-only config/rootfs, kernel WG 51824 for direct LAN bootstrap and a separate discovery proxy 51826, both with matching LAN-bound port mappings; existing upstream trial/services remain running. Owner raised exhausted kernel key-count quota to 512; byte quota remains 20000. Owner-generated phone peer authorized; actual WG handshake and bidirectional LAN traffic pass.
...
- LAN live acceptance: 32768-byte exact TCP echo from Android shell and from a separate app UID through the unchanged signed release; ordinary HTTPS/DNS work outside the VPN. Opt-in instrumentation 1/1 passes (0.563 s). No VPN default route or DNS takeover. During a controlled NAS trial outage, internet remained usable; tunnel recovered at the first successful sample after 20.88 s. A discovery refresh preserved the authenticated LAN endpoint. See dated public evidence and the corrected deployment port arrangement.

## Handover and persistence correction — release 4

Go d4d12e9 fixes stale-handshake retention, restores the trusted LAN bootstrap as a bounded recovery candidate and retains the existing backoff/scheduler. Full race/vet and actual image/kernel positive/negative WG gates pass. Android 3cfcb61 fixes Android JSON solidus escapes in the pinned parser without a dependency upgrade or weakened validation. The earlier hardware reload used a process cache; the new test decrypts and validates actual stored bytes.

Clean offline release4 build: 135 tasks, 12 debug +12 release tests, release lint, unchanged owner signer, verified native contents/16KiB alignment. Physical hardware/storage tests pass2/2 (0.983s); signed release restores the original encrypted identity after update; live split-route app probe passes1/1 (0.552s), and the tunnel HTTP canary passes. Updated hotspot→home retest passes on this NAT path; the owner estimates about10seconds to recover at home without restarting the app. External setup still waited for discovery; no exact outbound timing claim. Exact provenance/current limits are in `ARTIFACT_MANIFEST.json` and `DEVICE_TEST_RESULTS.md`; identifying physical-device evidence is retained privately. No new background worker, permission, host firewall or service-share change.
