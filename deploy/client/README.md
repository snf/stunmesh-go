...
Syncthing is not running on the inspected NAS. Follow the [fresh three-device plan](../../DIRECT_ACCESS_AND_SYNC_PLAN.md#2-fresh-syncthing-configuration-and-owner-run-enrollment) before activating it: new identities, reviewed folder/UID mappings, TCP-only sync and an authenticated temporary GUI through an SSH tunnel. No permanent GUI publication or VPN GUI forward is planned. Do not start the saved configurations unchanged or claim Syncthing access before testing it.
